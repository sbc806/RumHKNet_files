...
def get_total_ko_predictions():
  complete_small_df=get_dir_df("/home/schen123/projects/rrg-guanuofa/schen123/kinases/predictions/predicted_results/step_2/both/clustered/total_ko_small_kinase")
  large_df=pd.read_csv("/home/schen123/projects/rrg-guanuofa/schen123/kinases/predictions/predicted_results/step_2/both/clustered/2025_01_20_new_ko_shared_large_predicted_03.csv")
...
def analyze_method_histidine(df):
  ...
  print(np.unique(df["seq_id"]).shape,np.unique(total_ko_df["seq_id"]).shape,np.unique(total_blastp3050_df["seq_id"]).shape)
  print()
  ...
  shared_ko_blastp3050=np.sum(total_ko_df["seq_id"].isin(total_blastp3050_df["seq_id"].values))
  shared_rumhknet_ko=np.sum(df["seq_id"].isin(total_ko_df["seq_id"].values))
  shared_rumhknet_blastp3050=np.sum(df["seq_id"].isin(total_blastp3050_df["seq_id"].values))
  shared_rumhknet_ko_blastp3050=np.sum(df["seq_id"].isin(total_ko_df["seq_id"].values)&df["seq_id"].isin(total_blastp3050_df["seq_id"].values))
  rumhknet_only=np.sum(~df["seq_id"].isin(total_ko_df["seq_id"].values)&~df["seq_id"].isin(total_blastp3050_df["seq_id"].values))
  rumhknet_ko_blastp3050=np.sum((df["seq_id"].isin(total_ko_df["seq_id"].values))|(df["seq_id"].isin(total_blastp3050_df["seq_id"].values)))
                                
  print("Number of predictions in common between KO and Blast:",shared_ko_blastp3050)
