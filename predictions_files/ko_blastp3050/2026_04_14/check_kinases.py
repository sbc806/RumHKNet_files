...
pd.DataFrame(blastp.values[~blastp_contained],header=["seq_id"]).to_csv("blastp_missing.csv",index=False)
pd.DataFrame(kofamscan.values[~kofamscan_contained],header=["seq_id"]).to_csv("kofamscan_missing.csv",index=False)
